...
<script type="text/javascript">			
	...
	<div class='container-fluid content'><ul class="breadcrumb">
		...
		<li class="active">Applicant Reports</li>
		</ul>
	</div>
		...
		<div class="col-md-12 wrapper-background">
			<nav class="navbar nav2 nav navbar-nav">
				<div class="container-fluid">
					<ul class="nav navbar-nav">
						<li><h3><a href="applicantReports.php?token=<?php echo $util; ?>" style="margin-left:.5em;">Applicant Report</a></h3></li>
					</ul>
					<ul class="nav navbar-nav pull-right">
						<li><a target="_blank" href="../fpdf/pdfApplicantReports.php" style="margin-left:2.5em; margin-top:.2em;"><span class="glyphicon glyphicon-print"> </span>&nbsp; Print Report</a></li>
					</ul>
				</div>
			</nav>
	
			<h4 class="alert-info well-lg instruction">Applicant Report </h4> 		
			<br />
		  ...
		  <?php
			...
			$sql = "SELECT *
					...
					WHERE a.basicId = b.applicantId and a.basicId = c.basicId AND
					applicantDate BETWEEN '$start' AND '$end' GROUP BY contactNumber ORDER BY applicantDate";
			echo "Applicant Records From $start To $end Contract";
			...
			while($row = mysql_fetch_array($res)){
				 ...
				 echo "<td>" . $row['basicEmail'] . "</td>"; 
				
				 echo "<td>" . $row['contactNumber'] . "</td>"; 
				 echo "<td>" . $row['applicantDate'] . "</td>"; 
				 echo "</tr>"; echo "<tr>"; 
			...
			$sql = "SELECT *
					FROM tbl_basic_info a, tbl_applicant b, tbl_contact_info c
					WHERE a.basicId = b.applicantId
					GROUP BY basicEmail";
			echo "List of All Applicants";
			echo "<table class='table table-hover'> 
				  <tr> 
				<th>Name</th> 
				<th>Email</th> 
			...
			mysql_close($con); 
			   ...
			   ?>
			   <form name="calz" method="post" target="_blank" action="../fpdf/pdfApplicantSummary.php" >
